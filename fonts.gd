...
var Font6 : PackedByteArray = PackedByteArray([
...
var Font16 : PackedByteArray = PackedByteArray([
...
var pC : Pointer = Pointer.new([])
var adr : Pointer = Pointer.new(_buffer, fontPosX * fontPosY * 128)
func pceFontPrintF(formatString : String, params : Array[Variant] = []) -> void:
	adr.data = _buffer
	# (y * screen_width) + x
	adr.address = (fontPosY * 128) + fontPosX
	var c := (formatString % params).to_utf8_buffer()
	pC.data = c
	pC.address = 0
	while pC.address < c.size():
		var i : int
		var sAdr : int = 0
		var ubyte_val : int = pC.value_ubyte()
		if ubyte_val >= 0x20 and ubyte_val < 0x80:
			i = ubyte_val - 0x20
		else:
			i = 0
		sAdr = (i & 15) + (i >> 4) * 16 * 16
		for y in range(fontHeight):
			var c1 : int = fontAddress[sAdr]
			for x in range(fontWidth):
				if c1 & 0x80:
					adr.set_ubyte(fontFgColour)
				elif fontBgClear == 0:
					adr.set_ubyte(fontBgColour)
				adr.inc()
				c1 <<= 1
			adr.inc(128 - fontWidth)
			sAdr += 16
		adr.dec(128 * fontHeight - fontWidth)
		pC.inc()

var fontPosX : int = 0
var fontPosY : int = 0
var fontWidth : int = 4
var fontHeight : int = 6
var fontFgColour : int = 3
var fontBgColour : int = 0
var fontBgClear : int = 0
var fontAddress := Font6

func pceFontSetType(type : int) -> void:
	const width := [5,8,4]
	const height := [10, 16, 6]
	var adr := [Fonts.Font6, Fonts.Font16, Fonts.Font6]
	if type < 3:
		fontWidth = width[type]
		fontHeight = height[type]
		fontAddress = adr[type]

func pceFontSetPos(x : int, y : int) -> void:
	fontPosX = x
	fontPosY = y

func pceFontSetBkColour(colour : int) -> void:
	if colour >= 0:
		fontBgColour = colour
		fontBgClear = 0
	else:
...
func pceFontSetTxColour(c : int) -> void:
	fontBgColour = c
